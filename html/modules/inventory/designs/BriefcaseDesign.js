    ...
    <div class="relative w-full h-full flex justify-center items-center overflow-hidden">
        ...
        <div class="relative rounded-[2.5vw] shadow-[0_2vw_4vw_rgba(0,0,0,0.95),inset_0_0_2vw_rgba(0,0,0,0.6)] border-4 p-[1.5vw] flex flex-col transition-all duration-500 hover:shadow-[0_2.5vw_5vw_rgba(212,175,55,0.3),inset_0_0_2vw_rgba(212,175,55,0.2)]" :class="dualInventoryOpen ? 'w-[90vw] h-[64vh] justify-start' : 'w-full h-full'" style="background: linear-gradient(165deg, #5a3d25 0%, #4a2f1a 20%, #3d2415 45%, #2d1810 70%, #3d2415 85%, #4a2f1a 100%), repeating-linear-gradient(90deg, rgba(90,61,37,0.3) 0px, rgba(90,61,37,0.3) 1px, transparent 1px, transparent 10vw); border-color: #2d1810; animation: float 6s ease-in-out infinite;">
            ...
            <div class="relative z-10 grid h-full" 
                ...
                <div v-if="dualInventoryOpen" class="row-start-1 row-span-2 col-start-2 rounded-[1.3vw] shadow-[0_0.8vw_1.6vw_rgba(0,0,0,0.95),inset_0_0.5vh_1vh_rgba(0,0,0,0.7),0_0_1.5vw_rgba(212,175,55,0.25)] border-2 flex flex-col overflow-hidden" style="background: linear-gradient(135deg, rgba(90,61,37,0.9), rgba(74,47,26,0.95), rgba(58,37,21,0.98)), repeating-linear-gradient(90deg, rgba(212,175,55,0.05) 0, rgba(212,175,55,0.05) 0.1vw, transparent 0.1vw, transparent 0.8vw); border-color: rgba(212,175,55,0.6);">
                    <div class="px-[1vw] py-[0.8vh] border-b-2 flex justify-between items-baseline" style="background: linear-gradient(90deg, rgba(90,61,37,0.95), rgba(74,47,26,0.98)); border-color: rgba(212,175,55,0.4); box-shadow: 0 2px 5px rgba(0,0,0,0.5), inset 0 1px 2px rgba(212,175,55,0.2);">
                        <div class="font-bold text-[0.75vw] uppercase tracking-[0.22em] text-amber-200" style="text-shadow: 0 2px 4px rgba(0,0,0,0.9);">📦 {{ dualInventoryTitle }}</div>
                        <div class="text-[0.6vw] uppercase text-amber-300/90">{{ secondInventoryItems.filter(isItemDefined).length }}/{{ secondInventoryItems.length }}</div>
                    </div>
                    
                    <div class="flex-1 overflow-y-auto min-h-0 p-[0.5vw] custom-scrollbar-briefcase-second" style="scrollbar-width: thin; scrollbar-color: rgba(212,175,55,0.95) rgba(74,47,26,0.9);">
                        <div class="grid grid-cols-5 gap-[0.4vw] auto-rows-min">
                            <div v-for="(item, index) in secondInventoryItems" :key="'second-' + index" :data-slot-index="'second-' + index" @mousedown="(e) => handleMouseDown(e, 'second-' + index)" @contextmenu.prevent="(e) => openContextMenu(e, item, 'second-' + index)" @mouseenter="hoveredItem = item?.id" @mouseleave="hoveredItem = null" class="relative rounded-lg border-2 flex items-center justify-center p-[0.35vw] cursor-grab transition-all hover:-translate-y-[0.25vh] hover:scale-105 hover:border-amber-400" style="aspect-ratio: 1/1; width: 100%;" :style="isItemDefined(item) ? 'background: linear-gradient(135deg, rgba(90,61,37,0.8), rgba(74,47,26,0.9)); border-color: rgba(212,175,55,0.4);' : 'background: linear-gradient(135deg, rgba(90,61,37,0.4), rgba(74,47,26,0.6)); border-color: rgba(212,175,55,0.25);'"
                            >
