// Forest Design Template - UPDATED with symmetrical layout, vertical buttons, tooltip fix, and context menu
export function generateForestTemplate() {
    return `
    <div class="relative w-full h-full flex justify-center overflow-visible" :class="dualInventoryOpen ? 'items-start pt-[10vh]' : 'items-center'">
        ...
        <div class="relative w-full h-full rounded-[2.5vw] shadow-[0_2vw_4vw_rgba(0,0,0,0.95),inset_0_0_2vw_rgba(0,0,0,0.8)] border-4 p-[1.5vw] flex flex-col overflow-visible" :class="dualInventoryOpen ? 'justify-start' : ''" style="background: linear-gradient(165deg, #3d2817 0%, #2d1810 15%, #1a0f08 35%, #0d0604 60%, #1a0f08 85%, #2d1810 100%), repeating-linear-gradient(90deg, rgba(61,40,23,0.3) 0px, rgba(61,40,23,0.3) 2px, transparent 2px, transparent 8vw); border-color: #4a3728;">
            ...
            <div class="relative z-10 grid h-full gap-[1.5vw]" 
                :class="dualInventoryOpen ? 'grid-cols-[1fr_1fr_auto] items-start' : 'grid-cols-[20%_1fr_20%] grid-rows-[1fr_1fr] gap-[1vw]'">
                
                <!-- Wallet (top-left) - Hidden in dual mode -->
                    ...
                    <div class="flex-1 overflow-y-auto min-h-0 custom-scrollbar-forest" :class="dualInventoryOpen ? 'p-[0.8vw]' : 'p-[1vw]'" style="scrollbar-width: thin; scrollbar-color: rgba(34,197,94,0.95) rgba(45,24,16,0.9);">
                        <div class="grid" :class="dualInventoryOpen ? 'grid-cols-5 gap-[0.5vw]' : 'grid-cols-5 gap-[0.6vw]'">
                            <div v-for="(item, index) in inventoryItems" :key="index" 
                                ...
                                @contextmenu="(e) => openContextMenu(e, item, index)"
                ...
                <!-- Second Inventory (dual mode) OR Stats (normal mode) -->
                    ...
                    <div class="flex-1 overflow-y-auto min-h-0 p-[0.8vw] custom-scrollbar-forest" style="scrollbar-width: thin; scrollbar-color: rgba(34,197,94,0.95) rgba(20,83,45,0.9);">
                        <div class="grid grid-cols-5 gap-[0.5vw]">
                            <div v-for="(item, index) in secondInventoryItems" :key="'second-' + index" 
                                ...
                                @contextmenu="(e) => openContextMenu(e, item, 'second-' + index)"
                                @mouseenter="hoveredItem = item?.id"
                                @mouseleave="hoveredItem = null"
                ...
                <!-- Vertical Button Column (order-3 for dual mode, replaces right column in normal mode) -->
                <div :class="dualInventoryOpen ? 'order-3' : 'col-start-3 row-start-2'" class="flex flex-col-reverse gap-[1vh] justify-end">
                    <!-- Close Button (always at bottom) -->
                    <button @click="handleClose" class="w-full aspect-square rounded-xl border-2 flex items-center justify-center transition-all hover:scale-105 hover:shadow-[0_0_1.5vw_rgba(220,38,38,0.8)]" style="background: linear-gradient(135deg, rgba(139,0,0,0.9), rgba(90,0,0,0.95)); border-color: rgba(220,38,38,0.6); box-shadow: 0 0.4vw 0.8vw rgba(0,0,0,0.9);">
                    ...
                    <!-- Dual Inventory Action Buttons (only in dual mode) -->
                    <template v-if="dualInventoryOpen">
                        <button @click="closeDualInventory" class="w-full px-[0.8vw] py-[1.2vh] rounded-xl border-2 font-bold text-[0.7vw] uppercase tracking-wider transition-all hover:scale-105 hover:shadow-[0_0_2vw_currentColor]" style="background: linear-gradient(135deg, rgba(20,83,45,0.95), rgba(5,46,22,0.98)); color: #d1fae5; border-color: rgba(52,211,153,0.7); box-shadow: 0 0.5vh 1vh rgba(22,163,74,0.6);">
                            ✅ Speichern
                        </button>
                        
                        <button @click="clearDualInventory" class="w-full px-[0.8vw] py-[1.2vh] rounded-xl border-2 font-bold text-[0.7vw] uppercase tracking-wider transition-all hover:scale-105 hover:shadow-[0_0_2vw_currentColor]" style="background: linear-gradient(135deg, rgba(74,51,36,0.95), rgba(61,40,23,0.98)); color: #fed7aa; border-color: rgba(217,119,6,0.6); box-shadow: 0 0.5vh 1vh rgba(217,119,6,0.5);">
                            🗑️ Entfernen
                        </button>
                    </template>
                </div>
                
                <!-- Keys (bottom-left) - Hidden in dual mode -->
        ...
        <!-- Global Tooltip (High Z-Index, Fixed Position) -->
        <Teleport to="body">
            <div v-if="hoveredItem" 
                class="fixed pointer-events-none z-[99999] px-[0.7vw] py-[0.4vh] rounded-lg text-[0.75vw] font-medium border-2 whitespace-nowrap shadow-[0_0.5vh_1.5vh_rgba(0,0,0,0.9)] animate-[fadeIn_0.15s_ease-out]" 
                :style="{
            ...
            >
                {{ inventoryItems.find(i => i?.id === hoveredItem)?.name || secondInventoryItems.find(i => i?.id === hoveredItem)?.name || 'Item' }}
            </div>
        </Teleport>
        ...
        <Teleport to="body">
            <Transition name="context-fade">
                <div v-if="contextMenuOpen" 
                    class="context-menu fixed z-[99998] rounded-xl overflow-hidden shadow-[0_1vh_3vh_rgba(0,0,0,0.95)] border-2 animate-[scaleIn_0.12s_ease-out]"
                    :style="{
                        left: contextMenuPosition.x + 'px',
                        top: contextMenuPosition.y + 'px',
                        background: 'linear-gradient(135deg, rgba(20,83,45,0.98), rgba(5,46,22,0.99))',
                ...
                >
                    <div class="flex flex-col">
                        <button @click="useItemFromContext" class="px-[1vw] py-[0.8vh] text-left text-[0.7vw] font-medium text-emerald-100 hover:bg-emerald-500/20 transition-colors border-b border-emerald-950/30 flex items-center gap-[0.5vw]">
                            <span class="text-[1.2vw]">✅</span>
                            <span>Benutzen</span>
                        </button>
                        <button @click="showItemDescription" class="px-[1vw] py-[0.8vh] text-left text-[0.7vw] font-medium text-emerald-100 hover:bg-emerald-500/20 transition-colors border-b border-emerald-950/30 flex items-center gap-[0.5vw]">
                            <span class="text-[1.2vw]">📝</span>
                            <span>Beschreibung</span>
                        </button>
                        <button @click="showItemInfo" class="px-[1vw] py-[0.8vh] text-left text-[0.7vw] font-medium text-emerald-100 hover:bg-emerald-500/20 transition-colors flex items-center gap-[0.5vw]">
                            <span class="text-[1.2vw]">ℹ️</span>
